...
def clone_status(update, context):
    with download_dict_lock:
        count = len(download_dict)
    if count == 0:
        currentTime = get_readable_time(time() - botStartTime)
        free = get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)
        message = 'No Active Clone Tasks !\n___________________________'
...
clone_status_handler = CommandHandler(BotCommands.StatusCommand, clone_status,
                                      filters=CustomFilters.authorized_chat | CustomFilters.authorized_user)
status_pages_handler = CallbackQueryHandler(status_pages, pattern="status")

dispatcher.add_handler(clone_status_handler)
